...
let currentGroupFilter = 'all';  // 학생 관리 탭의 조 필터 상태
...
async function doDeposit() {
  ...
  try {
    ...
    document.getElementById('deposit-box').style.display = 'none';
    showToast(currentStudent.name + '님에게 $' + amt.toLocaleString() + '을 적립했어요');
  } catch (e) {
...
function renderPurchase() {
  ...
  document.getElementById('purchase-who').textContent =
    currentStudent.name + ' 학생 · 보유 $' + currentStudent.balance.toLocaleString();
  ...
  items.forEach((it, idx) => {
    ...
    row.innerHTML =
      '<div class="item-icon-box"><i class="ti ti-gift"></i></div>' +
      '<div style="flex:1;"><div style="font-size:16px; font-weight:800;">' + escapeHtml(it.name) + '</div><div style="font-size:13px; color:var(--mute); font-weight:600;">$' + it.price.toLocaleString() + '</div></div>' +
      '<button class="qty-btn qty-minus" onclick="changeCart(' + idx + ',-1)" aria-label="빼기">-</button>' +
      '<span style="min-width:24px; text-align:center; font-size:16px; font-weight:800;" id="cart-qty-' + idx + '">0</span>' +
...
async function doPurchase() {
  ...
  try {
    ...
    currentStudent.balance = newBalance;
    showToast(currentStudent.name + '님 구매 완료, $' + total.toLocaleString() + '을 사용했어요');
    switchTab('main');
    renderScanResult();
  } catch (e) {
...
// ===== 학생 관리 =====
const GROUP_LABELS = { A: 'A조', B: 'B조', C: 'C조', D: 'D조' };
...
function renderStudents() {
  ...
  // idx는 항상 원본 students 배열 기준 (일괄 적립이 이 idx로 학생을 찾기 때문)
  const visible = students
    .map((s, idx) => ({ s, idx }))
    .filter(({ s }) => {
      if (currentGroupFilter === 'all') return true;
      if (currentGroupFilter === 'unassigned') return !s.group;
      return s.group === currentGroupFilter;
    });

  if (visible.length === 0) {
    wrap.innerHTML = '<div class="empty-state"><i class="ti ti-users"></i>해당 조에 학생이 없습니다</div>';
    return;
  }
  ...
  visible.forEach(({ s, idx }) => {
    ...
    const groupLabel = GROUP_LABELS[s.group] || '미배정';
    const groupClass = 'group-badge-' + (GROUP_LABELS[s.group] ? s.group : 'none');
    row.innerHTML =
      '<input type="checkbox" data-idx="' + idx + '" class="bulk-check" style="width:24px; height:24px; flex:0 0 24px; accent-color:#3182F6;">' +
      '<div style="flex:1; min-width:0;">' +
        '<div class="s-name">' + escapeHtml(s.name) + ' <span class="group-badge ' + groupClass + '">' + groupLabel + '</span></div>' +
        '<div class="s-id">' + escapeHtml(s.id) + '</div>' +
      '</div>' +
      '<div class="s-balance">$' + s.balance.toLocaleString() + '</div>';
...
async function bulkApply(sign) {
  ...
  if (!confirm(checks.length + '명에게 $' + amt.toLocaleString() + '을 ' + (sign > 0 ? '적립' : '차감') + '합니다. 진행할까요?')) return;
...
function renderItemManage() {
  ...
  items.forEach((it) => {
    ...
    row.innerHTML =
      ...
      '<span style="font-size:12px; color:var(--mute); font-weight:700;">$</span>' +
      '<button class="btn-spend" style="flex:0 0 76px; height:42px; padding:0; display:flex; align-items:center; justify-content:center; gap:5px; font-size:13px;" onclick="removeItem(\'' + it.docId + '\')"><i class="ti ti-trash"></i>삭제</button>';
    wrap.appendChild(row);
  });
...
function fetchLogPage(isFirst) {
  let query = db.collection('logs').orderBy('timestamp', 'desc').limit(LOG_PAGE_SIZE);
  if (lastLogDoc) query = query.startAfter(lastLogDoc);

  query.get().then(snap => {
    const wrap = document.getElementById('log-list');
    if (isFirst) wrap.innerHTML = '';
    removeLoadMoreButton();

    if (snap.empty) {
      if (isFirst) wrap.innerHTML = '<div class="empty-state"><i class="ti ti-receipt"></i>아직 거래 내역이 없습니다</div>';
      return;
    }
    ...
    snap.docs.forEach(d => {
      ...
      row.innerHTML =
        ...
        '<span class="log-meta"><span class="' + deltaClass + '">' + sign + '$' + Math.abs(log.delta).toLocaleString() + '</span> · ' + time + '</span>';
      wrap.appendChild(row);
    });

    lastLogDoc = snap.docs[snap.docs.length - 1];

    if (snap.docs.length === LOG_PAGE_SIZE) {
      const btn = document.createElement('button');
      btn.id = 'log-load-more';
      btn.className = 'btn-secondary';
      btn.style.width = '100%';
      btn.style.marginTop = '8px';
      btn.innerHTML = '<i class="ti ti-chevron-down" style="margin-right:6px;"></i>더 보기';
      btn.onclick = () => fetchLogPage(false);
      wrap.appendChild(btn);
    }
  }).catch(() => {
    if (isFirst) document.getElementById('log-list').innerHTML = '<div class="empty-state">불러오기에 실패했습니다</div>';
  });
}
