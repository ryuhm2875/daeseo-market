...
function renderStudents() {
  ...
  students.forEach((s, idx) => {
    ...
    row.innerHTML =
      '<input type="checkbox" data-idx="' + idx + '" class="bulk-check" style="width:22px; height:22px; flex:0 0 22px;">' +
      '<div class="avatar" style="width:40px; height:40px; font-size:13px;">' + escapeHtml(s.name.slice(0,2)) + '</div>' +
      '<div style="flex:1;"><div style="font-size:16px; font-weight:600;">' + escapeHtml(s.name) + '</div><div style="font-size:13px; color:#8B95A1;">' + escapeHtml(s.id) + '</div></div>' +
      '<span style="font-size:17px; font-weight:700; color:#3182F6;">' + s.balance.toLocaleString() + '</span>';
