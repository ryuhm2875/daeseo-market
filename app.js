...
function renderPurchase() {
  ...
  items.forEach((it, idx) => {
    ...
    row.innerHTML =
      ...
      '<div style="flex:1;"><div style="font-size:16px; font-weight:800;">' + escapeHtml(it.name) + '</div><div style="font-size:13px; color:var(--mute); font-weight:600;">' + it.price.toLocaleString() + ' 달란트</div></div>' +
...
function renderItemManage() {
  ...
  items.forEach((it) => {
    ...
    row.innerHTML =
      ...
      '<span style="font-size:12px; color:var(--mute); font-weight:700;">달란트</span>' +
      '<button class="btn-spend" style="flex:0 0 42px; height:42px; padding:0;" aria-label="삭제" onclick="removeItem(\'' + it.docId + '\')"><i class="ti ti-trash"></i></button>';
    wrap.appendChild(row);
  });
